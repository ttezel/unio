...
Unio.prototype.request = function (verb, resource, params, callback) {
    ...
    // Adapted ExpressJS
    function normalizeUri(path, keys, sensitive, strict) {
    ...
    resourceCandidates.some(function (candidate, index) {
        ...
        var rgxName = candidate.name && normalizeUri(candidate.name, null, null, true);
        var rgxPath = candidate.path && normalizeUri(candidate.path, null, null, true);
